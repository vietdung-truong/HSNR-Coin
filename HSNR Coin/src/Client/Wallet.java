...
public class Wallet {
	...
	public HashMap<String, TransactionOutput> UTXOs = new HashMap<String, TransactionOutput>(); // filtert Einträge aus
																								// der UTXO von Main und
																								// speichert nur unsere
																								// UTXO
	...
	public Wallet() {// Konstruktor
		...
		try {
			...
			File f = new File(userHome+"//testpublickey");
			keyfos = new FileOutputStream(f);
			keyfos.write(key);
			keyfos.close();
		} catch (IOException e) {
	...
	public void generateKeyPair() {//geneniert durch ECDSA und randomizer SHA1PRNG eine zufällige Kombination von PublicKey und PrivateKey
		try {
	...
	public float getBalance() {
		float total = 0;
		for (Map.Entry<String, TransactionOutput> item : Blockchain.UTXOs.entrySet()) { // für jeden Eintrag aus Main. UTXO
	...
	public Transaction sendFunds(PublicKey _recipient, float value) {
		...
		Transaction newTransaction = new Transaction(publicKey, _recipient, value, inputs); // publicKey ist von diesem
																							// Wallet. Es wird eine
																							// Transaktion generiert
